...
void client_(int client_sockfd, char *diskfile)
{
    ...
    Inode share_directory_inode;
    get_inode(5, &share_directory_inode);
    
    char user_operations[10];
    char user_name[128];
    char user_password[124];
    ...
    strcpy(system_command, buf);
    sscanf(system_command, "%s %s %s", user_operations, user_name, user_password);
    if (user_operations[0] == 'C')
    {
        // char name
        if (-1 == create_user(&root_of_system, user_name, user_password))
        {
            char quit_ouput[1024];
            strcpy(quit_ouput, "please enter the correct command");
            write(client_sockfd, quit_ouput, sizeof(quit_ouput));
            return;
        }
        if (-1 == enter_user(&root_of_system, &current_directory, user_name, user_password))
        {
            char quit_ouput[1024];
            strcpy(quit_ouput, "please enter the correct command");
            write(client_sockfd, quit_ouput, sizeof(quit_ouput));
    ...
    else if (user_operations[0] == 'D')
    {
        // remove_user(&root_of_system,user_name,user_password);
        if (-1 == remove_user(&root_of_system, user_name, user_password))
        {
            char quit_ouput[1024];
            strcpy(quit_ouput, "please enter the correct command");
            write(client_sockfd, quit_ouput, sizeof(quit_ouput));
        ...
        char quit_ouput[1024];
        strcpy(quit_ouput, "success delete");
        write(client_sockfd, quit_ouput, sizeof(quit_ouput));
        return;
    }
    else
    ...
    // char shift_name[252];
    strcpy(shift_name, user_name);
    strcpy(user_name_global, user_name);
    
    get_current_route(&current_directory, "", shift_name, begin_output);
    usr_root_inode = current_directory;
    // strcat(begin_output, user_operations);
    // strcat(begin_output, user_name);
    // strcat(begin_output, user_password);
    ...
    while (1)
    {
        ...
        // int exit_ = command_execute(client_sockfd, buf, diskfile);
        {
            ...
            get_current_route(&current_directory, "", shift_name, route);
            // printf("\nthe route is:\n%s\nend of the route\n", route);
